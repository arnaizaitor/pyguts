    ...
    def write_report(self, messages_by_location):
        ...
        with open(os.path.join(self.output_dir, self.output_file), 'w') as file:
            for location, messages in messages_by_location.items():
                file.write(f"\n********************* {location} *********************\n")
                if location.lower() == "general":  # Using .lower() for case-insensitive comparison
                    for message in messages:
                        file.write(f"{message.msg_id}:{message.symbol} - {message.msg}\n")
                else:    
                    for message in messages:
                        file.write(f"{message.msg_id}:{message.symbol} - {message.line}:{message.column}: {message.msg}\n")
